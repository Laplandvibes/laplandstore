...
const NB_COPY: ProductAdCopy = {
  ...
  sub: {
    ...
    'pt-BR': 'A Nordicbuddies faz produtos licenciados em Helsinque desde 2019: uma sacola de algodão, uma bolsa de passaporte onde cabe o celular, um gorro canelado em poliéster reciclado. A loja envia para o mundo todo e toda opção tem rastreio, então a encomenda segue você para casa em vez de encher a mala.',
    'zh-CN': 'Nordicbuddies 自 2019 年起在赫尔辛基制作授权角色单品:棉布托特包、能装下手机的护照包、再生聚酯罗纹针织帽。店铺全球发货,每种配送方式都可追踪,包裹随后寄到家,不必塞满行李箱。',
    ko: 'Nordicbuddies는 2019년부터 헬싱키에서 라이선스 캐릭터 제품을 만들어 왔습니다. 면 토트백, 휴대폰이 들어가는 여권 가방, 재활용 폴리에스터 골지 비니. 전 세계로 발송하며 모든 배송 방법에 추적이 붙으니, 캐리어를 채우는 대신 소포가 집으로 따라옵니다.',
    fr: 'Nordicbuddies fabrique des produits sous licence à Helsinki depuis 2019 : un tote bag en coton, une sacoche à passeport où tient le téléphone, un bonnet côtelé en polyester recyclé. La boutique expédie dans le monde entier et chaque option est suivie : le colis vous rejoint chez vous au lieu de remplir la valise.',
    it: 'Nordicbuddies realizza prodotti su licenza a Helsinki dal 2019: una borsa di cotone, una borsa portapassaporto in cui entra il telefono, un berretto a coste in poliestere riciclato. Il negozio spedisce in tutto il mondo e ogni opzione è tracciata, così il pacco ti raggiunge a casa invece di riempire la valigia.',
    nl: 'Nordicbuddies maakt sinds 2019 in Helsinki gelicentieerde figuurartikelen: een katoenen tas, een paspoorttasje waar je telefoon in past, een geribbelde muts van gerecycled polyester. De winkel verstuurt wereldwijd en elke optie is te volgen, dus het pakket komt je achterna in plaats van je koffer te vullen.',
    sv: 'Nordicbuddies har gjort licensierade figurprodukter i Helsingfors sedan 2019: en bomullskasse, en passväska där telefonen får plats, en ribbstickad mössa i återvunnen polyester. Butiken skickar över hela världen och alla alternativ är spårbara, så paketet följer efter hem i stället för att fylla resväskan.',
  },
  trust: [
    {
      icon: Globe,
